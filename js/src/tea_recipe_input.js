...
// * 모바일 기능 추후 fix
{
  ...
  // 변수 -----------------------------------------------------
  const elRecipeBox = document.querySelector('#recipeBox');
  const elContainer = elRecipeBox.querySelector('.container');
  const elRecipeWrap = elRecipeBox.querySelector('.recipe_wrap');
  
  const elRecipeBtn = elRecipeWrap.querySelector('.recipe_btn_pc');
  
  const elRecipeCont = elRecipeWrap.querySelector('.recipe_content');
  const elRecipeLi = elRecipeCont.querySelectorAll('li');
  const elRecipeLiArr = [...elRecipeLi];

  const slideStyle = elRecipeCont.style;
  const slideLen = elRecipeLiArr.length; // li 갯수
  ...
  // 슬라이드 너비 체크
  const fnCkSlide = () => {
    const _liWidth = parseInt(window.getComputedStyle(elRecipeLi[0]).width); // li width 추출
    const _liMaginRight = parseInt(window.getComputedStyle(elRecipeLi[0]).marginRight); // li margin_right 추출
  ...
  const fnBtnAction = () => {
    ...
    const _elNextBtn = elRecipeBtn.querySelector('.next');
    const _elPrevBtn = elRecipeBtn.querySelector('.prev');
  ...
  // 이벤트 -----------------------------------------------------
  // 버튼 클릭
  elRecipeBtn.addEventListener('click', e => {
    e.preventDefault();
    if(PERMISSION) {
      PERMISSION = false;
  ...
  // 모바일 터치
  // 시작점 파악
  elRecipeCont.addEventListener('touchstart', fnTouchStart);
  // 터치하는 동안 슬라이드가 따라오기
  elRecipeCont.addEventListener('touchmove', fnTouchMove);
